...
async fn test_envelope_ingest_and_processing() {
    ...
    let keys: Vec<serde_json::Value> = client
        .get(format!("{base_url}/api/internal/projects/{project_id}/keys"))
        .send().await.unwrap()
        .json().await.unwrap();
    ...
    assert!(issue_list[0]["title"].as_str().unwrap().contains("TypeError"));
...
async fn test_grouping_same_error_same_issue() {
    ...
    for i in 0..3 {
        ...
        client.post(format!("{base_url}/api/{pid}/envelope/"))
            .header("X-Sentry-Auth", format!("Sentry sentry_key={key}"))
            .body(envelope).send().await.unwrap();
    }
    ...
    let issues: serde_json::Value = client
        .get(format!("{base_url}/api/internal/projects/group-test/issues"))
        .send().await.unwrap()
        .json().await.unwrap();
    ...
    assert_eq!(issues["issues"].as_array().unwrap().len(), 2, "Different error should create new issue");
}
