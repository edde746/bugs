...
pub fn routes() -> Router<AppState> {
    Router::new()
        ...
        .route(
            "/api/internal/projects/{slug}/tags/{key}/values",
            get(list_tag_values),
        )
...
async fn list_tag_keys(
    ...
    let result: Vec<serde_json::Value> = keys
        .iter()
        .map(|(k, v)| serde_json::json!({"key": k, "values_seen": v}))
        .collect();
...
async fn health_ready(State(state): State<AppState>) -> Result<&'static str, StatusCode> {
